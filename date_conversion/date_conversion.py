...
def reset_hijri_format(date, raise_error=False):
    if date:
        ...
        if len(split_date) != 3:
            if raise_error:
                raise ValidationError(_("error in date format '%s'" % date))
            else:
                return date
        if len(split_date[0]) == 4:
            year = split_date[0]
            month = split_date[1] if len(split_date[1]) == 2 else ('0' + split_date[1])
            day = split_date[2] if len(split_date[2]) == 2 else ('0' + split_date[2])
        elif len(split_date[2]) == 4:
            year = split_date[2]
            month = split_date[1] if len(split_date[1]) == 2 else ('0' + split_date[1])
            day = split_date[0] if len(split_date[0]) == 2 else ('0' + split_date[0])
        else:
            if raise_error:
                raise ValidationError(_("error in date format '%s'" % date))
            else:
